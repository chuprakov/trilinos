...
namespace TSFExtended
{
  ...
  class SumOperator : public OpDescribableByTypeID<Scalar>,
  ...
  public:
                       ...
                       ) const 
    {
      ...
      left_.ptr()->apply(M_trans, x, y, alpha, beta);
      if (!subtraction_)
	{
	  right_.ptr()->apply(M_trans, x, applyRight.ptr().get(), alpha, 0.0); 
	}
      else 
	{
	  right_.ptr()->apply(M_trans, x, applyRight.ptr().get(), 
			      -1.0*alpha, 0.0);
	}
      //TSFCore::linear_combination(1, 1.0, &applyRight, 1.0, y);
      TSFCore::Vp_StV(y, 1.0, *(applyRight.ptr().get()));
    }
