...
namespace TSF
{
  ...
  class StrUtils
    ...
    public:
      ...
      static TSFArray<string> readFile(istream& is, char comment);

      /** split an input string that contains newlines into an array
       * of strings, one for each line */
      static TSFArray<string> splitIntoLines(const string& input);

      /** tokenize a file into whitespace-delimited tokens */
      static TSFArray<TSFArray<string> > tokenizeFile(istream& is, char comment);
