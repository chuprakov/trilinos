...
namespace TSF
{
  ...
  /** \relates TSFHashUtils standard interface for getting the hash code of
   * an object */

  template <class T> int hashCode(const T& x);

  /** \relates TSFHashUtils get the hash code of an int */
  template <> inline int hashCode(const int& x)
    {
      return x;
    }

  /** \relates TSFHashUtils  get the hash code of a double */
  template <> inline int hashCode(const double& x)
    {
      return (int) x;
    }

  /**\relates TSFHashUtils  get the hash code of a bool */
  ...
  /** \relates TSFHashUtils get the hash code od a string */
