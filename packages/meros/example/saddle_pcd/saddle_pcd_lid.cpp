  ...
  try
    {
      ...
      const Epetra_Map* velocityMap = new Epetra_Map(1538, 0, Comm); 
      const Epetra_Map* pressureMap = new Epetra_Map(704, 0, Comm); 
 

      // Read matrix and vector blocks into Epetra_Crs matrices
      Epetra_CrsMatrix* FMatrix(0);
      char * filename = "../../../../../packages/meros/example/data/ldc_stab/F.mm";
      ...
      filename = "../../../../../packages/meros/example/data/ldc_stab/Bt.mm";
      MatrixMarketFileToCrsMatrix(filename,
                                  *velocityMap, 
      ...
      filename = "../../../../../packages/meros/example/data/ldc_stab/B.mm";
      MatrixMarketFileToCrsMatrix(filename,
                                  *pressureMap, 
                                  *pressureMap, *velocityMap,
                                  BMatrix);

      Epetra_CrsMatrix* CMatrix(0);
      filename = "../../../../../packages/meros/example/data/ldc_stab/C.mm";
      MatrixMarketFileToCrsMatrix(filename,
                                  *pressureMap, 
      ...
      filename = "../../../../../packages/meros/example/data/ldc_stab/Fp.mm";
      MatrixMarketFileToCrsMatrix(filename,
                                  *pressureMap, 
                                  *pressureMap, *pressureMap,
                                  FpMatrix);
      ...
      filename = "../../../../../packages/meros/example/data/ldc_stab/Ap.mm";
      MatrixMarketFileToCrsMatrix(filename,
                                  *pressureMap, 
                                  *pressureMap, *pressureMap,
                                  ApMatrix);

      Epetra_CrsMatrix* QpMatrix(0);
      filename = "../../../../../packages/meros/example/data/ldc_stab/Qp.mm";
      MatrixMarketFileToCrsMatrix(filename,
                                  *pressureMap, 
                                  *pressureMap, *pressureMap,
                                  QpMatrix);
      ...
      Epetra_Vector* rhs_vel_epetra(0);
      filename = "../../../../../packages/meros/example/data/ldc_stab/rhs_vel.mm";
      MatrixMarketFileToVector(filename,
                               *velocityMap, 
                               rhs_vel_epetra);

      Epetra_Vector* rhs_press_epetra(0);
      filename = "../../../../../packages/meros/example/data/ldc_stab/rhs_press.mm";
      MatrixMarketFileToVector(filename,
                               *pressureMap, 
                               rhs_press_epetra);
      ...
      RefCountPtr<SpmdVectorBase<double> > rhs1
        = create_Vector(rcp(rhs_press_epetra, false), epetra_vs_press);
      RefCountPtr<SpmdVectorBase<double> > rhs2
        = create_Vector(rcp(rhs_vel_epetra, false), epetra_vs_vel);
...
//       RefCountPtr<Thyra::LinearOpBase<double> > tmpZZ = 
...
//       const LinearOperator<double> ZZ = tmpZZ;
      ...
      // notation).
      LinearOperator<double> blockOp = block2x2(FF, BBt, BB, CC);
      ...
      if(DEBUG > 1) 
        {
          ...
          LinearOperator<double> testCC = blockOp.getBlock(1,1);
          ...
          cerr << "testC domain: " << testCC.domain().dim() << endl;
          cerr << "testC range: " << testCC.range().dim() << endl;
        }
      ...
      aztecFParams->sublist("Forward Solve").set("Max Iterations", 200);
      aztecFParams->sublist("Forward Solve").set("Tolerance", 10e-15);
      ...
      aztecFParams->sublist("Forward Solve")
        .sublist("AztecOO Settings").set("Size of Krylov Subspace", 200);
      ...
      // forward solve settings
      aztecApParams->sublist("Forward Solve").set("Max Iterations", 200);
      aztecApParams->sublist("Forward Solve").set("Tolerance", 10e-15);
      // aztecOO solver settings
      ...
      RefCountPtr<ParameterList> aztecQpParams 
        = rcp(new ParameterList("aztecOOQpSolverFactory"), true);
      
      // forward solve settings
      aztecQpParams->sublist("Forward Solve").set("Max Iterations", 200);
      aztecQpParams->sublist("Forward Solve").set("Tolerance", 10e-15);
      // aztecOO solver settings
      ...
      if(DEBUG > 1)
        {
          // Print out the parameters we just set
          RefCountPtr<LinearOpWithSolveFactoryBase<double> > 
            aztecQpLowsFactory = rcp(new AztecOOLinearOpWithSolveFactory());
          aztecQpLowsFactory->setParameterList(aztecQpParams);
          aztecQpLowsFactory->getParameterList()->print(cerr, 0, true, false);
        }


      // 3) Make a PCDOperatorSource with blockOp, Fp, Ap, and Qp
      //    The pressure mass matrix Qp is the identity in this example.
      // RefCountPtr<const LinearOpSourceBase<double> > myPCDopSrcRcp 
      //   = rcp(new PCDOperatorSource(blockOp, Fp, Ap));
      RefCountPtr<const LinearOpSourceBase<double> > myPCDopSrcRcp 
	= rcp(new PCDOperatorSource(blockOp, Fp, Ap, Qp));
