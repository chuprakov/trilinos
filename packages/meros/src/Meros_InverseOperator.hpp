...
#ifndef MEROS_INVERSE_OPERATOR_H
#define MEROS_INVERSE_OPERATOR_H
...
namespace Meros 
{
  ...
  /** \brief 
   * Clean interface for creating a Thyra inverse operator
   */
  template <class Scalar>
  class InverseOperator
    : public Handleable<LinearOpBase<Scalar> >,
      public DefaultInverseLinearOp<Scalar>
  {
  public:
    /** */
    InverseOperator(const LinearOperator<Scalar>& op,
                    const LinearSolveStrategy<Scalar>& solver)
      : DefaultInverseLinearOp<Scalar>(solver.getLOWS(op))
    {
    }
    
    /* */
    /** */
    InverseOperator(const ConstLinearOperator<Scalar>& op,
                    const LinearSolveStrategy<Scalar>& solver)
      : DefaultInverseLinearOp<Scalar>(solver.getLOWS(op))
    {
    }
