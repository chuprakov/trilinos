...
## "owner1@abc.com, owner2@def.com"
    echo "didasko-regression@software.sandia.gov" >>& $file
...
foreach f ( amesos anasazi aztecoo epetra epetraext ifpack ml nox teuchos triutils)
    ...
    if ( ${exefiles} != 'fakefile.exe' ) then
	  ...
	  foreach g(*.exe)
		  ...
		  if( "$2" == "True" ) then
		      ...
		      if( $status != 0 ) then
		        # A test failed.
			    set AnError = True
			    echo "  ******** Serial Test w/ 1 proc failed ********" >>& ../$file
			    echo "Errors for script " $g " are listed above." >>& ../$file2
		      else
		        # Tests passed
			    echo "******** Serial Test w/ 1 proc passed ********" >>& ../$file
		      endif
		  else
      ...
      echo "*** Test ${f} did not run, no executable found ***" >>& ../$file
      echo "******** Perhaps a package the test depends on didn't build? ****" >>& ../$file
      rm -f fakefile.exe
    endif
	cd ..
end
