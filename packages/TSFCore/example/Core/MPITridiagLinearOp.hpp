...
#ifndef TSFCORE_MPI_TRIDIAG_LINEAR_OP_HPP
#define TSFCORE_MPI_TRIDIAG_LINEAR_OP_HPP

#include "TSFCoreMPILinearOpBase.hpp"
...
public:
	...
	MPITridiagLinearOp( MPI_Comm mpiComm, const TSFCore::Index localDim, const Scalar lower[], const Scalar diag[], const Scalar upper[] )
	...
	void initialize(
		MPI_Comm                        mpiComm
		,const TSFCore::Index           localDim // >= 2
...
protected:
	...
	bool opSupported( TSFCore::ETransp M_trans ) const { return (M_trans == TSFCore::NOTRANS); }
	...
	void euclideanApply(
		...
		) const
		{
			...
			TEST_FOR_EXCEPTION( M_trans != TSFCore::NOTRANS, std::logic_error, "Error, can not handle transpose!" );
			// Get constants
...
#endif	// TSFCORE_MPI_TRIDIAG_LINEAR_OP_HPP
