...
int main(int argc, char *argv[])
{
	...
	try {
		...
		RefCountPtr<TSFCore::LinearOp<double> >
		...
		RefCountPtr<const TSFCore::VectorSpace<double> > b_space;
		if(useWithNonEpetraVectors) {
			...
			RefCountPtr<const TSFCore::EpetraVectorSpace> 
				A_range = Teuchos::rcp_dynamic_cast<const TSFCore::EpetraVectorSpace>(A->range(),true); // Throw if fail!
			b_space = Teuchos::rcp(new TSFCore::MPIVectorSpaceStd<double>(A_range->mpiComm(),A_range->localSubDim(),-1));
			// Note, this new vector space will create vectors of concrete type TSFCore::MPIVectorStd which are
			// compatible but not the same as the concrete type TSFCore::EpetraVector created by EpetraVectorSpace
		}
		else {
			// Have the RHS vector b use the same vector space as the operator A range space
			b_space = A->range();
			// Note, this vector space will create vectors of concrete type TSFCore::EpetraVector
		}
		// (D.3) Create the RHS vector b and initialize it to a random vector
		RefCountPtr<TSFCore::Vector<double> > b = b_space->createMember();
		TSFCore::seed_randomize<double>(0);
		TSFCore::randomize( -1.0, +1.0, &*b );
		// (D.4) Create LHS vector x and set to zero
		RefCountPtr<TSFCore::Vector<double> > x = A->domain()->createMember();
		TSFCore::assign( &*x, 0.0 );
		//
		// (E) Solve the linear system with the silly CG solver
		//
		result = sillyCgSolve(TSFCore::LinearOpHandle<double>(A),*b,maxNumIters,tolerance,&*x,verbose?&out:0);
