...
Teuchos::RefCountPtr<TSFCore::LinearOp<double> > createTridiagEpetraLinearOp(
	const int      globalDim
#ifdef RTOp_USE_MPI
	,MPI_Comm      mpiComm
#endif
	...
	)
{
  ...
  // Note that Epetra_CrsMatrix is *not* a handle object so have to use RefCountPtr above.
	...
	// into a TSFCore::EpetraLinearOp object to turn it into a TSFCore::LinearOp object
	//

	RefCountPtr<TSFCore::LinearOp<double> > A = rcp(new TSFCore::EpetraLinearOp(A_epetra));

	//
	// (D) Finally return the TSFCore-wrapped Epetra matrix object
	//
	
	return A;

	// Note that when this function returns the returned
	// RefCountPtr-wrapped TSFCore::LinearOp object will own all of the
	...
	// allocated TSFCore::LinearOp object are removed and destruction
	// occurs!
