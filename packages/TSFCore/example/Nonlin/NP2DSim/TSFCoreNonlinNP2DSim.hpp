	...
	: isInitialized_(false), a_(a), b_(b), d_(d), lin_sol_tol_(lin_sol_tol), c_(NULL), DcDy_(NULL)
{
	...
	if(space_y_c.get()) {
		THROW_EXCEPTION( !(space_y_c->dim()==2), std::logic_error, "NP2DSim<Scalar>::NP2DSim(...): Error!" );
		space_y_c_ = space_y_c;
	}
	else {
		space_y_c_ = mmp::rcp(new SerialVectorSpace<Scalar>(2));
	}
	...
	) const
{
	...
	mmp::ref_count_ptr<MultiVector<Scalar> >
		DcDy_mv = mmp::rcp_dynamic_cast<MultiVector<Scalar> >(
			mmp::rcp_const_cast<LinearOp<Scalar> >(DcDy_state.M)
			);
	mmp::ref_count_ptr<Solvers::BiCGSolver<Scalar> >
		DcDy_solver = mmp::rcp_dynamic_cast<Solvers::BiCGSolver<Scalar> >(
			mmp::rcp_const_cast<Solvers::IterativeLinearSolver<Scalar> >(DcDy_state.solver)
			);
	mmp::ref_count_ptr<Solvers::ConvergenceTester<Scalar> >
		convTester = DcDy_state.convTester;
	mmp::ref_count_ptr<MultiVector<Scalar> >
		DcDy_prec = mmp::rcp_dynamic_cast<MultiVector<Scalar> >(
			mmp::rcp_const_cast<LinearOp<Scalar> >(DcDy_state.M_tilde_left_inv)
			);
	// Create these objects if they have not been created already
	if( !DcDy_mv.get() )      DcDy_mv     = this->space_y()->createMembers(2);
	if( !DcDy_solver.get() )  DcDy_solver = mmp::rcp(new Solvers::BiCGSolver<Scalar>());
	if( !convTester.get() )   convTester  = mmp::rcp(new Solvers::NormedConvergenceTester<Scalar>(lin_sol_tol_));
	// Get at the state vector and Jacobian data
	if(true) {
		ExplicitVectorView<Scalar>           y(y_in);
		mmp::ref_count_ptr<Vector<Scalar> >  Dc1Dy_vec = DcDy_mv->col(1),  Dc2Dy_vec = DcDy_mv->col(2);
	...
	if( !DcDy_prec.get() ) {
		...
		mmp::ref_count_ptr<Vector<Scalar> >  Dc1Dy_prec_vec = DcDy_prec->col(1),  Dc2Dy_prec_vec = DcDy_prec->col(2);
