 ...
 * subset of elements in any <tt>const Vector</tt> object.
 *
 * <b>Warning!</b> Creating an explicit view of an arbitrary
 * <tt>%Vector</tt> object may be a very expensive operation (such as
 ...
 * of elements from in any <tt>const Vector</tt> object and copy
 ...
 *
 ...
 void copyToArrayPointerUnit(
   const TSFCore::Vector<Scalar>   &v
   ,const TSFCore::Range1D         &rng
   ,Scalar                         x[]   // Size == rng.size()
   )
 {
    TSFCore::ExplicitVectorView<Scalar> v_ev(rng,v,true);  // Force unit stride
 ...
 {
    TSFCore::ExplicitVectorView<Scalar> v_ev(rng,v);         // Allow non-unit stride
    const Scalar          *v_ptr    = v_ev.values();         // Get pointer to non-unit-stride data
    const TSFCore::Index  *v_stride = v_ev.stride();         // Get stride between vector data
 ...
 \endcode
 *
 * If one wants to modify the elements in a <tt>Vector</tt> object
...
public:
	 ...
	 * </ul>
	 */
	ExplicitVectorView( const Vector<Scalar>& v, const Range1D &rng = Range1D(), const bool forceUnitStride = false )
		:v_(v)
		{
	...
	/// Free the explicit view on the <tt>Vector</tt> object <tt>v</tt> passed to <tt>ExplicitVectorView()</tt>
	~ExplicitVectorView()
		{
 ...
 *
 * This utility class makes it easy to explicitly access a contiguous
 * subset of elements in any <tt>Vector</tt> object and change the
 * <tt>Vector object</tt>.
 *
 * <b>Warning!</b> Creating an explicit view of an arbitrary
 * <tt>%Vector</tt> object may be a very expensive operation (such as
 ...
 * of elements from in any <tt>Vector</tt> object and then add to them
 ...
 *
 ...
 void addToArrayPointerUnit(
   ...
   ,TSFCore::Vector<Scalar>        *v
   )
 {
    TSFCore::ExplicitMutableVectorView<Scalar> v_ev(rng,*v,true);  // Force unit stride
    const Scalar *v_ptr = v_ev.values();                           // Get pointer to unit-stride data
    for( int k = 0; k < n; ++k )                                   // For each element in view:
 ...
 {
    TSFCore::ExplicitMutableVectorView<Scalar> v_ev(rng,*v); // Allow non-unit stride
    const Scalar          *v_ptr    = v_ev.values();         // Get pointer to non-unit-stride data
    const TSFCore::Index  *v_stride = v_ev.stride();         // Get stride between vector data
    for( int k = 0; k < rng.size(); ++k, v_ptr += v_stride ) // For each element in view:
 ...
 \endcode
 *
 * If one wants to only read the elements in a <tt>Vector</tt> object
...
public:
	 ...
	 * </ul>
	 */
	ExplicitMutableVectorView( Vector<Scalar>& v, const Range1D &rng = Range1D(), const bool forceUnitStride = false )
		:v_(v)
		{
			v_.getSubVector(rng,&sv_s_);
			if( forceUnitStride && sv_s_.stride() != 1 ) {
	...
	/// Commits back the the explicit view on the <tt>Vector</tt> object <tt>v</tt> passed to <tt>ExplicitMutableVectorView()</tt>
