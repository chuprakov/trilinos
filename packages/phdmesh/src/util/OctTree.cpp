...
ostream & operator << ( ostream & os , const phdmesh::OctTreeKey & key )
{
  unsigned j = 0 ;

  while ( j < phdmesh::OctTreeKey::MaxDepth ) {
    os << key.index(++j);
  }
...
void OctTreeKey::throw_index( const unsigned d , const unsigned i ) const
{
  std::ostringstream msg ;
  msg << "phdmesh::OctTreeKey index[" ;
  msg << d ;
  msg << "] = " ;
  msg << i ;
  msg << " is out of range [1..8]" ;
  throw std::range_error( msg.str() );
}

void OctTreeKey::throw_depth( const unsigned depth ) const
{
  const unsigned m = MaxDepth ;
  std::ostringstream msg ;
  msg << "phdmesh::OctTreeKey depth = " ;
  msg << depth ;
  msg << " is out of range [1.." ;
  msg << m ;
  msg << "]" ;
  throw std::range_error( msg.str() );
}
...
OctTreeKey & OctTreeKey::clear()
{
  const unsigned zero = 0 ;
  Copy<NWord>( m_value , zero );
  return *this ;
}

OctTreeKey & OctTreeKey::set_maximum()
{
  enum { OK = StaticAssert< IndexPerWord == 8 >::OK };
  static const unsigned max = 0x88888888 ;
  Copy<NWord>( m_value , max );
  return *this ;
}

unsigned OctTreeKey::index( const unsigned Depth ) const
{
  if ( Depth < 1 || MaxDepth < Depth ) { throw_depth( Depth ); }
...
unsigned oct_tree_size( const unsigned Depth )
{
  if ( 10 < Depth ) {
    std::ostringstream msg ;
    msg << "phdmesh::oct_tree_size( depth = " ;
    msg << Depth ;
    msg << " ) is out of range [0..10]" ;
    throw std::range_error( msg.str() );
  }
...
unsigned oct_tree_offset( const unsigned Depth , const OctTreeKey & k )
{
  if ( 10 < Depth ) {
    std::ostringstream msg ;
    msg << "phdmesh::oct_tree_offset( depth = " ;
    msg << Depth ;
    msg << " , ... ) is out of range [0..10]" ;
    throw std::range_error( msg.str() );
  }
