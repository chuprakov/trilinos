                       ...
                       std::ostream & msg )
{
  ...
  static const char method[] = "phdmesh::CommAll::communicate" ;
...
{
  static const char method[] = "phdmesh::CommAll::communicate" ;
  static const int mpi_tag = PHDMESH_MPI_TAG_DATA ;
...
void CommBuffer::pack_overflow() const
{
  ...
  os << "phdmesh::CommBuffer::pack<T>(...){ overflow by " ;
  os << remaining() ;
  os << " bytes. }" ;
...
{
  std::ostringstream os ;
  os << "phdmesh::CommBuffer::unpack<T>(...){ overflow by " ;
  os << remaining();
  os << " bytes. }" ;
  throw std::overflow_error( os.str() );
}
    ...
    m_recv(NULL)
{
  ...
  if ( NULL == m_send ) {
    std::string msg("phdmesh::CommAll::CommAll FAILED malloc");
...
void CommAll::swap_send_recv()
{
  if ( m_recv == NULL ) {
    // ERROR
    std::string
      msg("phdmesh::CommAll::swap_send_recv(){ NULL recv buffers }" );
...
bool CommAll::allocate_buffers( ParallelMachine comm ,
			        ...
			        const bool local_flag )
{
  static const char method[] = "phdmesh::CommAll::allocate_buffers" ;
...
{
  static const char method[] = "phdmesh::CommBroadcast::allocate_buffer" ;
...
{
  static const char method[] = "phdmesh::CommBroadcast::send_buffer" ;
...
void CommBroadcast::communicate()
{
#if defined( PHDMESH_HAS_MPI )
  {
    ...
    if ( MPI_SUCCESS != result ) {
      std::ostringstream msg ;
      msg << "phdmesh::CommBroadcast::communicate ERROR : "
          << result << " == MPI_Bcast" ;
      throw std::runtime_error( msg.str() );
    }
  }
...
bool comm_dense_sizes( ParallelMachine comm ,
                       ...
                       bool local_flag )
{
  static const char method[] = "phdmesh::comm_dense_sizes" ;
...
bool comm_sizes( ParallelMachine comm ,
                 ...
                 bool local_flag )
{
  static const char method[] = "phdmesh::comm_unknown_sizes" ;
  const unsigned uzero = 0 ;
  ...
  if ( num_msg_bound < max_msg ) {
    ...
    const int mpi_tag = PHDMESH_MPI_TAG_SIZING ;
